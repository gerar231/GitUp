...
class UserAccount(object):
    """
    This class manages information about the currently logged in user (and their
    remote repositories) for interacting with the UI and Daemon.
    """
    ...
    def __init__(self, user_name: str=None, password: str=None,
        ...
        Arguments:
            ...
            token_file_path: opens the file at this path and reads the first line
                for an authorization token, if provided.
        If a token_file_path is provided:
            Checks the file given at path for an authorization token, if none or
                invalid path then throw ValueError. 
            If a token exists and is invalid then throw a ValueError.
        If user_name and password are provided:
            Logs in to a user account and creates a token for GitUp that will be
            stored with the scopes of "user, delete_repo, repo" at the path
            /tmp/GitUp/token.txt.
            If a token for GitUp already exists at the path /tmp/GitUp/token.txt
            then login to that user.
        
        If no arguments provided:
            searches default path /tmp/GitUp/token.txt for token, if no token exists
        ...
        if token_file_path:
            ...
            if not os.path.exists(token_file_path) and not user_name and not password:
                raise ValueError("No token file at default path {}"
        ...
        if existing_token:
            if user_name and password:
                print(ValueError("A user token already exists, cannot login to a"
                         + " new user."))
            ...
            try:
                ...
                raise(ValueError("Invalid token provided in first line of file at {}"
                        .format(token_file_path)))
        ...
        if user_name and password:
            ...
            # get existing authorizations if for GitUp then delete,
            ...
            for auth in authorizations:
                if auth.note_url == "https://github.com/gerar231/GitUp":
                    auth.delete()
            # create a new token
            token = self.__github_control.get_user().create_authorization(
                    scopes=["user", "delete_repo", "repo"],
    ...
    def get_remote_repos(self):
        ...
        representing the name's and clone urls of the current user's remote
        repositories.
        """
        repos = list()
    ...
    def create_remote_repo(self, local_repo: Repo):
        """
        Arguments:
            local_repo: GitPython Repo object to create a remote repository on
                the user account.
        Create a remote repository under the name of the parent directory 
        containing the repo on the current user's GitHub account and add remote
        under the name "GitUp" to this local repository. 
        Adds all changes and pushes all contents of local repo into remote repo
        after creation.
        ...
        try:
            ...
            # attempt to create a new remote repository using the folder containing
            # the repository name
            repo_name = os.path.basename(os.path.normpath(
                    os.path.join(local_repo.common_dir, "..")))
            ...
            for curr_repo in existing_repos:
                if curr_repo.name == repo_name:
                    raise AssertionError("Existing remote repo found with the name {}."
                            .format(repo_name))

            # create a new remote repository
            remote_repo = self.__github_control.get_user().create_repo(
                    name=repo_name, 
                    description="Repository managed by GitUp.")
            ...
            # perform a push using the git binary, specifying the url
            # dynamically generated in the above format
            if not local_repo.git.push(self.__create_push_url(local_repo, "GitUp"),
                    "master"):
                raise exc.GitCommandError("Push to origin failed after remote repo" + 
                        " created for {}".format(os.path.join(local_repo.common_dir, "..")), 1)
    ...
    def __create_push_url(self, local_repo: Repo, remote_name: str):
        ...
        """
        # get the "GitUp" remote url, assumes url is the only one and is the first from
        # iterator
    ...
    def push_to_remote(self, local_repo):
        ...
        try:
            ...
            raise exc.GitCommandError("No GitUp remote for repo {}."
                    .format(local_repo.working_tree_dir), 1)
        # push to the remote
        if not local_repo.git.push(self.__create_push_url(local_repo, "GitUp"), "master"):
            raise exc.GitCommandError("Push to GitUp remote failed for repo {}."
                    .format(local_repo.working_tree_dir), 1)
    ...
    def pull_to_local(self, local_repo: Repo):
        ...
        try:
            ...
            raise exc.GitCommandError("No GitUp remote for repo {}."
                    .format(local_repo.working_tree_dir), 1)
        # push to the remote
        if not local_repo.remote(name="GitUp").pull():
            raise exc.GitCommandError("Pull to repo {} from remote GitUp failed."
                    .format(local_repo.working_tree_dir), 1)
